...
{
    class WebsitetoTextExample
    {
        private static readonly string API_KEY = Environment.GetEnvironmentVariable("APIVERVE_API_KEY") ?? "YOUR_API_KEY_HERE";
        private static readonly string API_URL = "https://api.apiverve.com/v1/websitetotext";

        /// <summary>
        /// Make a POST request to the Website to Text API
        /// </summary>
        static async Task<JsonDocument> CallWebsitetoTextAPI()
        {
            try
            {
        ...
        {
            Console.WriteLine("📤 Calling Website to Text API...\n");

            var result = await CallWebsitetoTextAPI();
